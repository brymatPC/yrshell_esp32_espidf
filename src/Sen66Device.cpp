...
void Sen66Device::setup() {
    ...
    uint32_t _handle;
    uint8_t temp;
    size_t len = SENSIRION_STATE_LEN;
    ...
    memset(m_vocState, 0, SENSIRION_STATE_LEN);
    err = nvs_get_blob(_handle, "voc", m_vocState, &len);
    if(err != ESP_OK) {
        ESP_LOGE(TAG, "nvs_get_blob fail: voc - %lu", err);
    }
    nvs_close(_handle);
}
void Sen66Device::save() {
    ...
    err = nvs_set_blob(_handle, "voc", m_vocState, SENSIRION_STATE_LEN);
    if(err != ESP_OK) {
...
void Sen66Device::loadVoc() {
    bool vocValid = false;
    for(uint8_t i=0; i < SENSIRION_STATE_LEN; i++) {
        if(m_vocState[i] != 0x00) {
            vocValid = true;
            break;
        }
    }
    if(vocValid) {
        int16_t error = sen66_set_voc_algorithm_state(m_vocState, SENSIRION_STATE_LEN);
        if (error != NO_ERROR) {
...
void Sen66Device::toJson(char *buf, uint32_t maxLen, bool addLineEnding) {
    if(!buf) return;
    char ending[3] = "\r\n";
    if(!addLineEnding) {
        ending[0] = '\0';
    ...
    char ts[51];
    getRtcTimeStr(ts, 51);
    snprintf(buf, maxLen, "{\"ts\":\"%s\",\"up\":%lu,\"sn\":\"%s\",\"pm1\":%u,\"pm2\":%u,\"pm4\":%u,\"pm10\":%u,\"t\":%d,\"h\":%d, \"voc\":%d,\"nox\":%d,\"co2\":%u}%s",
        ts, (HW_getMillis()-m_resetTimeMs), m_serialNumber, pm1p0, pm2p5, pm4p0, pm10p0, temperature, humidity, vocIndex, noxIndex, co2, ending);
}
...
void Sen66Device::writeReadings() {
    ...
    toJson(m_logBuf, MAX_SEN66_SEND_BUF_SIZE, true);
    ...
    switch(m_state) {
        ...
        case STATE_READ_STATE:
            ...
            if (error != NO_ERROR) {
                ...
                ESP_LOGD(TAG, "VocState: 0x%02X%02X%02X%02X%02X%02X%02X%02X", m_vocState[7], m_vocState[6], m_vocState[5], m_vocState[4], m_vocState[3], m_vocState[2], m_vocState[1], m_vocState[0]);
            }
            m_state = STATE_IDLE;
        break;
        case STATE_UPLOAD:
        ...
        case STATE_UPLOAD_WAIT:
            if(!m_uploadClient) {
                ...
                toJson(m_sendBuf, MAX_SEN66_SEND_BUF_SIZE, false);
