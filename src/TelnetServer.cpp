    ...
    if( m_lastConnected && m_client < 0) {
      ...
      switch( m_state) {
        case STATE_STARTUP:
          ...
          if(m_configured) {
            ...
            if(m_listenSocket < 0) {
              ...
              //setsockopt(m_listenSocket, IPPROTO_IPV6, IPV6_V6ONLY, &opt, sizeof(opt));
              ...
              //memcpy(dest_addr.sin6_addr.s6_addr + 11, (uint8_t *)&_addr[0], 4);
        ...
        case STATE_IDLE:
        {
          ...
          m_client = accept(m_listenSocket, (struct sockaddr *)&_client, (socklen_t *)&cs);
          if(m_client >= 0) {
            int keepAlive = 1;
            int keepIdle = 5;
            int keepInterval = 5;
